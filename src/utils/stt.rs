...
impl WhisperSTT {
    pub fn new(
        ...
        Self {
            ...
            output_text_history: output_text_history,
    ...
    fn run(mut self) {
        ...
        while !self.is_ui_closed.load(Ordering::Acquire) {
            // get path
            let new_path_model = self.select_model.lock().unwrap().clone();
            ...
            if let Some(inside_state) = self.state.as_mut() {
                ...
                if self.buffer_live.len() >= VEC_MAXIMUM_SAMPLES {
                    full_text_history = Self::task_whisper(
                        inside_state,
                        params,
                ...
                if !new_full_text.trim().is_empty() || !full_text_history.trim().is_empty() {
                    let mut output = self.output_text.lock().unwrap();
                    *output = new_full_text;

                    let mut output_h = self.output_text_history.lock().unwrap();
...
pub fn worker(
    ...
    let mut model_file: PathBuf = PathBuf::new();
    ...
    while !is_ui_closed.load(Ordering::Acquire) {
        // get path
        let new_path_model = select_model.lock().unwrap().clone();

        // check if path and file is valid and change model if is different name
        if let Some(path) = new_path_model && path != model_file {
            // check if file is available in that path
            if !std::fs::exists(&path).unwrap_or(true) {
                eprintln!(
                    "No file found in this path: {}\nPlease go to settings to pick file path for stt model",
        ...
        if let Some(inside_state) = state.as_mut() {
            ...
            if buffer_live.len() >= VEC_MINIMUM_SAMPLES {
                new_full_text = task_whisper(inside_state, params.clone(), &mut buffer_live);
            }
